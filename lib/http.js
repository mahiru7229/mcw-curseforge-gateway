...
function cacheControl(policy) {
    ...
    const values = [
        ...
        `s-maxage=${policy.sMaxAge ?? 0}`,
    ];
    if (policy.staleWhileRevalidate) {
        values.push(`stale-while-revalidate=${policy.staleWhileRevalidate}`);
    }
    return values.join(", ");
}
...
function mapUpstreamError(error, requestId) {
    ...
    if (status === 401 || status === 403) {
        return jsonResponse({ error: { code: "gateway_not_configured", message: "The CurseForge gateway credentials are unavailable.", requestId } }, 503);
    }
...
export async function handleApiRequest(request, handler, options = {}) {
    ...
    try {
        if (options.requireClientAuth) {
            assertClientAuthorized(request);
        }
        ...
        if (error instanceof CurseForgeUpstreamError) {
            console.error("CurseForge upstream request failed", { requestId, status: error.upstreamStatus, message: error.message });
