...
export async function curseForgeRequest(path, options = {}) {
    ...
    const url = new URL(`${getCurseForgeBaseUrl()}${requestPath}`);
    ...
    const headers = new Headers({
        ...
        "X-Request-Id": options.requestId ?? "gateway-request",
    ...
    try {
        ...
        throw new CurseForgeUpstreamError(`Network request failed: ${message}`, 502, null, { path: requestPath });
    }
    ...
    if (!response.ok) {
        ...
        throw new CurseForgeUpstreamError(
            `CurseForge returned HTTP ${response.status}.`,
            response.status,
            response.headers.get("retry-after"),
            { path: requestPath, responsePreview: preview },
        );
    }
    ...
    try {
        ...
        throw new CurseForgeUpstreamError("CurseForge returned invalid JSON.", 502, null, { path: requestPath });
    }
}
